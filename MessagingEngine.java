...
import  org.wso2.carbon.andes.dataAgent.dataAgent.DataAgent;
import  org.wso2.carbon.andes.dataAgent.configure.Dataoutput;
import org.wso2.carbon.andes.dataAgent.dataAgent.AutoSender;
...
public class MessagingEngine {
    ...
    public void messageReceived(final AndesMessageMetadata message, long channelID) throws AndesException {
        try {
            if (message.isTopic) {
            ...
            //TODO : angal need to remove and run automatically

            AutoSender autoSenderObject = new  AutoSender();
            autoSenderObject.dataAgent("BAM");

            //call to dataAgent using a thread

            Dataoutput dataoutputObjectBAM = new Dataoutput("BAM");
            if(dataoutputObjectBAM.isEnable()) {
                Thread dAgentThread = new Thread(new Runnable() {
                @Override
                public void run() {
                DataAgent dAgent = new DataAgent();
                    try {
                        dAgent.dataAgent(message, "BAM" , noOfSubscriptions);
                    }
                    catch (Exception e){
    ...
    public void ackReceived(final AndesAckData ack) {
    	disruptorBasedExecutor.ackReceived(ack);

        //System.out.print("ack received");


        //send stats to bam and cep when ack received

       Dataoutput dataoutputObjectBAMACK = new Dataoutput("BAM");
        if(dataoutputObjectBAMACK.isEnable()) {
            Thread dAgentThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
